...
import static org.leandreck.endpoints.processor.model.typefactories.TypeNodeKind.COLLECTION;
import static org.leandreck.endpoints.processor.model.typefactories.TypeNodeKind.PagingAndFilterResponse;
/**
 */
public final class TypeNodeFactory {
    ...
    private TypeNode initType(final String fieldName, final String parameterName, final boolean optional, final TypeNodeKind typeNodeKind, final TypeMirror typeMirror, final DeclaredType containingType, final String doc) {
        ...
        if (nodes.containsKey(key) && !TypeKind.TYPEVAR.equals(typeMirror.getKind())) {
        	if (!typeNodeKind.equals(OPTIONAL) && !typeNodeKind.equals(COLLECTION) && !typeNodeKind.equals(PagingAndFilterResponse)) {
    ...
    private TypeNodeKind defineDeclaredTypeNodeKind(final TypeMirror typeMirror) {
        ...
        final TypeMirror pagingAndFilterResponseMirror = typeUtils.getDeclaredType(elementUtils.getTypeElement("com.kirana.avatar.common.dto.PagingAndFilterResponse"));
        ...
        } else if (typeUtils.isAssignable(typeMirror, typeUtils.erasure(pagingAndFilterResponseMirror))) {
        	typeNodeKind = TypeNodeKind.PagingAndFilterResponse;
        } else {
