...
export class SellerTruckDriverComponent implements OnInit {
    ...
    ngOnInit() {
        console.info( "OnInit SellerTruckDriver Component tns" );
        this.sellerOrders = [];
    ...
    protected showDetailDialog(sellerOrder: any) {
        let ref: DynamicDialogRef = this.dialogService.open(SellerTruckDriverOrderComponent, {
            data: {
                sellerOrder: sellerOrder, qualities: this.qualities
            },
            header: sellerOrder.seller.firstName + ' ' + sellerOrder.priceTag.product[this.languageCode],
            width: '50%',
            height: '70%'
        });
