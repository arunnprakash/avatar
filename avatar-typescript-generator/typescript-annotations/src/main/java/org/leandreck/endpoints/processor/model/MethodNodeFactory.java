...
class MethodNodeFactory {
    ...
    MethodNode createMethodNode(final ExecutableElement methodElement, final DeclaredType containingType) {
        ...
        System.out.println("getting a method ReturnType");
        final TypeNode returnType = defineReturnType(methodElement, containingType);
        System.out.println("Done getting a method ReturnType");
        ...
        System.out.println("Created a method " + name + " with return type " + returnType);
    ...
    private List<TypeNode> definePathVariableTypes(final List<? extends VariableElement> parameters, final DeclaredType containingType) {
    	System.out.println("definePathVariableTypes");
	...
	private String getFieldName(VariableElement it){
		//return it.getAnnotation(PathVariable.class).name() != null ? it.getAnnotation(PathVariable.class).name() : it.getSimpleName().toString();
		System.out.println("PathVariable Name " + it.getAnnotation(PathVariable.class).name());
		System.out.println("PathVariable Value " + it.getAnnotation(PathVariable.class).value());
		System.out.println("VariableElement Name " + it);
		return it.getAnnotation(PathVariable.class).name();
	}
    ...
    private static List<String> defineHttpMethods(final RequestMapping requestMapping) {
        ...
        if (requestMapping != null) {
            return Arrays.stream(requestMapping.method())
                    .map(requestMethod -> requestMethod.toString().toLowerCase(Locale.ENGLISH))
                    .collect(toList());
        }
